...
import {
  ...
  LandingPage, NotFound, UserProfile, OrganizationProfile, EventProfile,
} from './views';

import {
  RegisteredAndLoggedInRoute, LoginRoute, OrganizationRoute, ProtectedRoute,
  RegisterRoute,
} from './routes/index';
...
function App( props ){
  ...
  useEffect( () => {
    if( state.auth.googleAuthUser && state.auth.googleAuthUser.uid ){
      const orgSub = subscribeToUserOrganizations( state.auth.googleAuthUser.uid,
        dispatch,
      );
      ...
      setSubscriptions( {
        orgSub, [ state.auth.googleAuthUser.uid ]: messageSub,
      } );
    }
  ...
  useEffect( () => {
    state.org.userOrganizations.forEach( org => {
      if( !subscriptions[ org.orgId ] ){
        const messageSub = subscribeToMessages( {
          type: 'organizations', uid: org.orgId,
        }, dispatch );
    ...
    <Layout style={ { background: '#fafafa' } }>
      ...
      <Layout style={ { background: '#fafafa' } }>
        <HeaderDiv loggedIn={ state.auth.loggedIn }>
          { state.auth.loggedIn && state.auth.signedUp &&
          ( <MenuButton collapsed={ collapsed }
                        setCollapsed={ setCollapsed }/> ) }
          <LoginText hidden={state.auth.loggedIn} to={ '/login' }>Login</LoginText>
        </HeaderDiv>
        <NavPageMask
          collapsed={ collapsed }
